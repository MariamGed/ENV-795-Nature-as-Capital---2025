...
rm(list=ls())
```
Special thanks to a Andie Creel and previous Nature as Capital TF whose R Tutorial's inspired this version.
...
Another useful package for working with Tables (DataFrames) in R is dplyr, as part of package tidyverse. You can install dplyr directly (install.packages("dplyr")), or by installing tidyverse package install.packages("tidyverse").
Once you install it, explore the functionality of %>% i.e. pipe operator.

Tables:
You will process data mostly in the table format. For the most part, you’ll be loading new data in as tables, for example by reading/importing .CSV, or .xlsx into the R document. Before I explain this, let's first explore how you can create on your own. By looking at how they’re created, we can get a better sense of what goes into them.
...
You can import data as a table from your device, by reading the file. Common files that hold such data are .csv and .xlxs. 

```{r, results='hide'}

my_data <- read.csv("Path to the .csv file on your computer") #reading a csv file

For excel document, you first install a package:
install.packages("readxl")
library(readxl) # load the library
 
my_data2 <- read_excel('"Path to the .xlsx file on your computer") #reading an .xlxs file 

```
There are also built-in  datasets on R environment. Accessing them is very simple and these datasets are often used in tutorials. 
Visit this website for directions on importing them: https://www.sthda.com/english/wiki/r-built-in-data-sets 
